...
export class MothershipActor extends Actor {
  ...
  getFlavorText(type, context, action) {
    ...
    if (game.settings.get("mosh-fork", "useCalm") && context === 'stress') {
      context = 'calm';
    }
  ...
  parseRollResult(rollString, rollResult, zeroBased, checkCrit, rollTarget, comparison, specialRoll) {
    ...
    let useCalm = game.settings.get('mosh-fork', 'useCalm');
    let die0value = 999;
    let die1value = 999;
  ...
  async rollTable(tableId, rollString, aimFor, zeroBased, checkCrit, rollAgainst, comparison) {
    ...
    let firstEdition = game.settings.get('mosh-fork', 'firstEdition');
    let useCalm = game.settings.get('mosh-fork', 'useCalm');
    let androidPanic = game.settings.get('mosh-fork', 'androidPanic');
    ...
    //customize this roll if its a unique use-case
      ...
      if (tableId === 'panicCheck') {
        ...
        if (firstEdition) { 
          if (androidPanic && this.system.class.value.toLowerCase() === 'android') { 
            if (useCalm) {
            tableId = game.settings.get('mosh-fork', 'table1ePanicCalmAndroid');
              aimFor = 'low';
              zeroBased = true;
              checkCrit = true;
            ...
            tableId = game.settings.get('mosh-fork', 'table1ePanicStressAndroid');
              aimFor = 'high';
              zeroBased = false;
              checkCrit = false;
              rollAgainst = 'system.other.stress.value';
            ...
            if (useCalm) { 
            tableId = game.settings.get('mosh-fork', 'table1ePanicCalmNormal');
              aimFor = 'low';
              zeroBased = true;
              checkCrit = true;
              rollAgainst = 'system.other.stress.value';
            ...
            tableId = game.settings.get('mosh-fork', 'table1ePanicStressNormal');
              aimFor = 'high';
              zeroBased = false;
              checkCrit = false;
              rollAgainst = 'system.other.stress.value';
          ...
          if (androidPanic && this.system.class.value.toLowerCase() === 'android') { 
            if (useCalm) { 
            tableId = game.settings.get('mosh-fork', 'table0ePanicCalmAndroid');
              aimFor = 'low';
              zeroBased = true;
              checkCrit = true;
              rollAgainst = 'system.other.stress.value';
            ...
            tableId = game.settings.get('mosh-fork', 'table0ePanicStressAndroid');
              aimFor = 'high';
              zeroBased = false;
              checkCrit = false;
              rollAgainst = 'system.other.stress.value';
            ...
            if (useCalm) { 
            tableId = game.settings.get('mosh-fork', 'table0ePanicCalmNormal');
              aimFor = 'low';
              zeroBased = true;
              checkCrit = true;
              rollAgainst = 'system.other.stress.value';
            ...
            tableId = game.settings.get('mosh-fork', 'table0ePanicStressNormal');
              aimFor = 'high';
              zeroBased = false;
              checkCrit = false;
              rollAgainst = 'system.other.stress.value';
      ...
      if (tableId === 'maintenanceCheck') {
        ...
        specialRoll = tableId;
        //assign variables
      tableId = game.settings.get('mosh-fork', 'table1eMaintenance');
        zeroBased = true;
        checkCrit = true;
    ...
    if (game.settings.get('mosh-fork', 'panicDieTheme') != "") { //We're going to check if the theme field is blank. Otherwise, don't use this.
        //set panic die color
      let dsnTheme = game.settings.get('mosh-fork', 'panicDieTheme');
    ...
    flavorText = this.getFlavorText('table', tableName.replaceAll('& ', '').replaceAll(' ', '_').toLowerCase(), 'success');
      ...
      if (useCalm && !parsedRollResult.success && parsedRollResult.critical) {
        tableResultFooter = `<br><br>You lose 1d10 Calm because you critically failed.<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.jHyqXb2yDFTNWxpy]{-1d10 Calm}`;
      }
      ...
      if (specialRoll === 'maintenanceCheck' && !useCalm && !parsedRollResult.success && !parsedRollResult.critical) {
        tableResultFooter = `<br><br>Everyone on board the ship takes 1 Stress.<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.dvJR9DYXI2kV0BbR]{+1 Stress}`;
      }
      //append effects for Stress + Critical Maintenance Check Failure
      if (specialRoll === 'maintenanceCheck' && !useCalm && !parsedRollResult.success && parsedRollResult.critical) {
        tableResultFooter = `<br><br>Everyone on board the ship takes 1 Stress. You must roll for another maintenance issue because you critically failed.<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.dvJR9DYXI2kV0BbR]{+1 Stress}<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.hRapiXGVW8WZQH12]{Roll for Maintenance Issue}`;
      }
      //append effects for Calm + Maintenance Check Failure
      if (specialRoll === 'maintenanceCheck' && useCalm && !parsedRollResult.success && !parsedRollResult.critical) {
        tableResultFooter = `<br><br>Everyone on board the ship loses 1d10 Calm.<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.jHyqXb2yDFTNWxpy]{-1d10 Calm}`;
      }
      //append effects for Calm + Critical Maintenance Check Failure
      if (specialRoll === 'maintenanceCheck' && useCalm && !parsedRollResult.success && parsedRollResult.critical) {
        tableResultFooter = `<br><br>Everyone on board the ship loses 1d10 Calm. You must roll for another maintenance issue because you critically failed.<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.jHyqXb2yDFTNWxpy]{-1d10 Calm}<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.hRapiXGVW8WZQH12]{Roll for Maintenance Issue}`;
      }
      //append effects for Calm + Critical Maintenance Check Success
      if (specialRoll === 'maintenanceCheck' && useCalm && parsedRollResult.success && parsedRollResult.critical) {
        flavorText = flavorText + ` Gain 1d10 Calm.<br><br>@UUID[Compendium.mosh-fork.macros_triggered_1e.k2TtLFOG9mGaWVx3]{+1d10 Calm}`;
      }
    ...
    if (tableResult[0].type === 0 || tableResult[0].type === 'text') {
      ...
      //prepare template
      messageTemplate = 'systems/mosh-fork/templates/chat/rollTable.html';
      //render template
  ...
  async chooseAttribute(rollString, aimFor) {
    ...
    return new Promise(async (resolve) => {
      ...
      //create HTML for this window
        //header
      let dialogDesc = await foundry.applications.handlebars.renderTemplate('systems/mosh-fork/templates/dialogs/skill-check-stat-selection-dialog.html');
  ...
  async chooseSkill(dlgTitle, rollString) {
    ...
    return new Promise(async (resolve) => {
      ...
      //create HTML for this window
        //header
        let skillHeader = await foundry.applications.handlebars.renderTemplate('systems/mosh-fork/templates/dialogs/choose-skill-dialog-header.html');
        //skill template
        let skillRow = `
        <label for="[RADIO_ID]">
  ...
  async rollCheck(rollString, aimFor, attribute, skill, skillValue, weapon,overrideDamagaRollString=null) {
    ...
    if (specialRoll === 'damage') {  
      ...
      let dsnTheme = 0;
      if (game.settings.get('mosh-fork', 'damageDiceTheme') != "") { //We're going to check if the theme field is blank. Otherwise, don't use this.
        //set damage dice color
        dsnTheme = game.settings.get('mosh-fork', 'damageDiceTheme');
      }
      //prepare flavortext
      ...
      //generate chat message
        ...
        //create message
        const template = 'systems/mosh-fork/templates/chat/rollCheck.html';
    ...
    //set chat message text
      ...
      if (game.settings.get('mosh-fork', 'damageDiceTheme') != "") { //We're going to check if the theme field is blank. Otherwise, don't use this.
        ...
        if (game.settings.get('mosh-fork', 'critDamage') === 'advantage') {
            parsedDamageString = '{' + parsedDamageString + ',' + parsedDamageString + '}kh';
        } else if (game.settings.get('mosh-fork', 'critDamage') === 'doubleDamage') {
            critMod = ' * 2';
        } else if (game.settings.get('mosh-fork', 'critDamage') === 'doubleDice') {
            critMod = ' + ' + parsedDamageString + '[' + dsnTheme + ']';
        } else if (game.settings.get('mosh-fork', 'critDamage') === 'maxDamage') {
          parsedDamageString = parsedDamageString.replaceAll('d', ' * ');
        } else if (game.settings.get('mosh-fork', 'critDamage') === 'weaponValue') {
            critMod = ' + ' + weapon.system.critDmg + '[' + dsnTheme + ']';
        } else if (game.settings.get('mosh-fork', 'critDamage') === 'none') {
            //do nothing
          }
        }
        ...
        if (parsedRollResult.success === true) {
          ...
          if (firstEdition) {
            //if calm not enabled
            if (!useCalm) {
              if (game.settings.get('mosh-fork', 'autoStress')) { //If the automatic stress option is enabled
      ...
      //prepare flavor text for special rolls
        ...
        if (specialRoll === 'restSave') {
          ...
          //override  header image
          msgImgPath = `systems/mosh-fork/images/icons/ui/macros/rest_save.png`;
          //prepare attribute label
          attributeLabel = this.system.stats[attribute].label;
          //1e rest save
          if (firstEdition) {
        ...
        if (specialRoll === 'bankruptcySave') {
          ...
          //set header image
          msgImgPath = 'systems/mosh-fork/images/icons/ui/rolltables/bankruptcy_save.png';
          //prepare attribute label
          attributeLabel = game.i18n.localize("Mosh.Bankrupcy");
          //get the bankruptcy table
          let tableId = game.settings.get('mosh-fork','table1eBankruptcy');
          //get Table Data
          let tableData = await fromIdUuid(tableId,{type:"RollTable"});
        ...
        if (specialRoll === 'moraleCheck') {
          ...
          //set header image
          msgImgPath = 'systems/mosh-fork/images/icons/ui/macros/morale_check.png';
          //prepare attribute label
      ...
      //prepare flavor text for regular checks
        ...
        //set header image
        msgImgPath = 'systems/mosh-fork/images/icons/ui/attributes/' + attribute + '.png';
        //prep text based on success or failure
        if (parsedRollResult.success === false && this.type === 'character') {
          //if first edition
          if (firstEdition) {
      ...
      let messageData = {
        ...
        woundEffect: woundEffect,
        critFail: critFail,
      firstEdition: game.settings.get('mosh-fork', 'firstEdition'),
      useCalm: game.settings.get('mosh-fork', 'useCalm'),
      androidPanic: game.settings.get('mosh-fork', 'androidPanic')
      };
      //prepare template
      messageTemplate = 'systems/mosh-fork/templates/chat/rollCheck.html';
      //render template
      messageContent = await foundry.applications.handlebars.renderTemplate(messageTemplate, messageData);
      //make message
      let macroMsg = await rollResult.toMessage({
      ...
      if (!firstEdition && (-1 * modifyChange) > (modifyMaximum / 2)) {
        ...
        if (outputChatMsg) {
          //generate chat message
            ...
            //prepare template
            messageTemplate = 'systems/mosh-fork/templates/chat/modifyActor.html';
            //render template
            messageContent = await foundry.applications.handlebars.renderTemplate(messageTemplate, messageData);
            //push message
            ChatMessage.create({
  ...
  async modifyItem(itemId, addAmount) {
    ...
    //generate chat message
      ...
      //prepare template
      messageTemplate = 'systems/mosh-fork/templates/chat/modifyItem.html';
      //render template
      messageContent = await foundry.applications.handlebars.renderTemplate(messageTemplate, messageData);
      //make message
      ChatMessage.create({
  ...
  async reloadWeapon(itemId) {
    ...
    //generate chat message
      ...
      //prepare template
      messageTemplate = 'systems/mosh-fork/templates/chat/reload.html';
      //render template
      messageContent = await foundry.applications.handlebars.renderTemplate(messageTemplate, messageData);
      //push message
      ChatMessage.create({
  ...
  async takeCryoDamage(rollString) {
    ...
    <div class="mosh-fork">
      <div class="rollcontainer">
          ...
          <div style="text-align: right"><img class="roll-image" src="systems/mosh-fork/images/icons/ui/attributes/health.png" /></div>
          </div>
          <div class="description"" style="margin-bottom: 20px;">
          <div class="body">
          ${msgFlavor}
  ...
  async chooseCover() {
    ...
    return new Promise(async (resolve) => {
      ...
      //create pop-up HTML
      let msgContent = await foundry.applications.handlebars.renderTemplate('systems/mosh-fork/templates/dialogs/choose-cover-dialog.html', {
          curDR:curDR, 
          curAP:curAP, 
  ...
  async distressSignal() {
    ...
    return new Promise(async (resolve) => {
      //create pop-up HTML
      let msgContent = await foundry.applications.handlebars.renderTemplate('systems/mosh-fork/templates/dialogs/distres-signal-dialog.html');
        ...
        buttons: [
          {
            label: game.i18n.localize("Mosh.Advantage"),
			      action: `action_advantage`,
            callback: () => this.rollTable(game.settings.get('mosh-fork', 'table1eDistressSignal'), `1d10 [+]`, `low`, true, false, null, null),
            icon: `fas fa-angle-double-up`
          },
          {
            label: game.i18n.localize("Mosh.Normal"),
  ...
  async chatDesc(item) {
    ...
    const template = 'systems/mosh-fork/templates/chat/itemRoll.html';
